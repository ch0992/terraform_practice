...
#위에서 생성한 IAM Role에 Policy를 추가한다. (AmazonEKSWorkerNodePolicy)
...
#위에서 생성한 IAM Role에 Policy를 추가한다. (AmazonEKS_CNI_Policy)
resource "aws_iam_role_policy_attachment" "terraform-eks-node-AmazonEKS_CNI_Policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
  role = aws_iam_role.terraform-eks-node.name
}

#위에서 생성한 IAM Role에 Policy를 추가한다. (AmazonEC2ContainerRegistryReadOnly)
...
output "print_subnet_ids" {
    value = aws_subnet.terraform-eks-private-subnet[*].id
}
